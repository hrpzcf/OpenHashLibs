...
static inline void
add512(const uint512_u_t *x, const uint512_u_t *y, uint512_u_t *r)
{
...
static void
g(uint512_u_t *h, const uint512_u_t *N, const unsigned char *m)
{
    ...
    /* Starting E() */
    Ki = data;
    XLPS((&Ki), ((const uint512_u_t *) &m[0]), (&data));
    ...
    X((&data), h, (&data));
    X((&data), ((const uint512_u_t *) &m[0]), h);
#endif
}
...
stage3(GOST34112012Context *CTX)
{
    ...
    add512(&(CTX->Sigma), (const uint512_u_t *) &CTX->buffer[0],
           &(CTX->Sigma));
